...
const UI_DEBUG_VERSION = import.meta.env.VITE_APP_VERSION || 'local-dev';
  ...
  return (
    <div className="h-screen bg-gray-50 flex flex-col">
      ...
      <div className="fixed bottom-20 right-4 max-w-md bg-white border border-gray-200 rounded-lg shadow-lg z-40">
        ...
        >
          API Debug {debugInfo ? (debugInfo.ok ? '✅' : '❌') : 'ℹ️'} {debugInfo ? `(${debugInfo.status})` : '(no requests yet)'} {debugOpen ? '▲' : '▼'}
        </button>

        {debugOpen && (
          <div className="px-4 py-3 text-xs text-gray-700 space-y-1">
            <p><span className="font-semibold">UI Version:</span> {UI_DEBUG_VERSION}</p>
            {debugInfo ? (
              <>
              ...
              <p>No chat API request has completed yet. Send one prompt to populate diagnostics.</p>
            )}
          </div>
        )}
      </div>
